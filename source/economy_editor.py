...
class EconomyEditorWindow(tk.Frame):
    ...
    def __init__(self, master, project: "mp_mod.ModProject", on_change=None):
        ...
        # Pristine (clean-backup) copies for the DEFAULT value shown beside the current one. Loaded
        # best-effort — if there's no backup, defaults are simply omitted (helpers return None).
        self._gd_clean = None
        self._gd_clean_obj = None
        self._clean_ndf = None
        self._clean_bldg = {}   # ClassNameForDebug -> clean building instance
    ...
    def _index_clean_defaults(self):
        """Load the pristine gdconstante + everything NDFs and index clean buildings by
        ClassNameForDebug, so each field can show its original (clean-backup) value. No-ops gracefully
        (defaults omitted) when no backup is configured."""
        try:
            self._gd_clean = self.project.clean_ndf("gameplay", mp_mod.GDCONSTANTE_PATH)
            if self._gd_clean is not None and self._gd_clean.instances:
                self._gd_clean_obj = self._gd_clean.instances[0]
        except Exception:
            self._gd_clean = self._gd_clean_obj = None
        try:
            self._clean_ndf = self.project.clean_everything()
        except Exception:
            self._clean_ndf = None
        if self._clean_ndf is not None:
            for inst in self._clean_ndf.class_instances("TBatimentDescriptor"):
                v = self._pv(self._clean_ndf, inst, "ClassNameForDebug")
                if v is not None and isinstance(v.raw, int):
                    self._clean_bldg[self._clean_ndf.get_string(v.raw)] = inst

    def _gd_default(self, prop):
        """The default (clean-backup) NdfValue for a global-constant prop, or None."""
        if self._gd_clean_obj is None:
            return None
        return self._pv(self._gd_clean, self._gd_clean_obj, prop)

    def _bldg_default(self, prop):
        """The default (clean-backup) NdfValue for `prop` on the selected building, matched by
        ClassNameForDebug, or None (no backup / a building the mod added)."""
        clean = self._clean_bldg.get(self._descr_name(self._b_sel)) if self._b_sel is not None else None
        return self._pv(self._clean_ndf, clean, prop) if clean is not None else None
        ...
        for gtitle, fields in _GLOBAL_GROUPS:
            ...
            for lbl, prop, kind in present:
                if kind == "derived":
                    ...
                    self._row(frame, self._g_rows, lbl, prop, kind,
                              self._pv(self._gd, self._gd_obj, prop), self._gd_default(prop))
    ...
    def _build_bldg_tab(self, parent):
        ...
        self._b_rows = []
        self._b_sel = None
        # Debounced load + instant 'Loading …' feedback (see ui_util.debounce_load): a click-drag down
        # the list loads only the settled selection, and what loads matches the highlighted row.
        ui_util.debounce_load(self._b_lb, self._on_bldg_select, on_peek=self._peek_bldg)
    ...
    def _derived_row(self, container, label, key):
        ...
        out.grid(row=0, column=3, sticky="e", padx=(0, 4))
    ...
    def _on_bldg_select(self, _=None):
        ...
        for lbl, prop, kind in _BLDG_FIELDS:
            ...
            if val is not None:
                self._row(self._b_fields, self._b_rows, lbl, prop, kind, val, self._bldg_default(prop))
            covered.add(prop)
        ...
        self._b_reset.configure(state="normal")
