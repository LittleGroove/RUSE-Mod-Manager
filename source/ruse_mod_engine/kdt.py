...
# ── CS node-opcode + TIL codecs (reverse-engineered from the KDT runtime format) ──
# Both proven byte-identical round-trip on beta/chess/blitz.
...
# Reverse-engineered from the KDT IndexBuffer format (decode) — the per-index reader the
...
def decode_mesh(kdt_bytes: bytes):
    """Decode the KDT's triangle mesh OFFLINE for any map (CRACKED 2026-05-24; validated against the game).
    The 85/etc verts in d1 are NOT absolute — they're RESIDUALS + back-ref instructions decoded by the
    VtxBufIdx predictor (reverse-engineered from the vertex-index stream format):
...
def rebuild_kdt_tree(kdt_bytes: bytes, verts_world):
    """v2 FULL rebuild of the KDT for the 'KDT follows sectors' edit.
