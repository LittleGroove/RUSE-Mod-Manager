...
class UnitsEditorWindow(tk.Frame):
    ...
    def _add_field_row(self, container, rows, label, prop, kind, val):
        ...
        # "factory" current value shows which actual building(s) host this unit (Nat + TypeBatiment
        # join can match multiple — e.g. Prototype + Atomic both have TypeBatiment=12). Useful when
        # a unit isn't appearing where expected — the host list reveals what the engine sees.
        if kind == "factory" and self._sel is not None and isinstance(val.raw, int):
            hosts = self._buildings_for_factory(self._sel["nation"], val.raw)
            host_names = ", ".join(b["name"] for b in hosts) if hosts else "(no matching building)"
            cur_display = f"{val.raw} ({_FACTORY_TYPE.get(val.raw, '?')}): {host_names}"
        else:
            cur_display = cur
        # Truncate the in-grid preview to keep the row aligned EXCEPT for the factory row, whose
        # value lists building names that the user needs to see in full to diagnose menu wiring.
        if kind == "factory":
            ...
            tk.Label(row, text=cur_display[:self._VAL_W], width=self._VAL_W, anchor="e",
                     background=_R_BG_PANEL, foreground=_R_TEXT_DIM, font=_F_MAIN
                     ).grid(row=0, column=1, sticky="e", padx=(0, 12))
