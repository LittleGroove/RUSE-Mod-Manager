...
def val_to_rmod(ndf, val, new_inst_indices=None, orig_ndf=None):
    ...
    if t == T.Reference:
        ...
        if marker == ndfbin_mod.TRANS_REF_MARKER:
            # Resolve to string name so the index is portable across NDF files
            return ("TransRef", {"trans": ndf.get_trans(ref)})
        return None
...
def _impr_expr_differ(orig_ndf, mod_ndf) -> bool:
    """True if IMPR or EXPR sections differ semantically (resolved TRAN strings)."""
    def _resolve_list(ndf, lst):
        result = []
        for idx in lst:
            try:
                result.append(ndf.get_trans(idx))
            except Exception:
                result.append(f"__raw_{idx}__")
        return result

    if _resolve_list(orig_ndf, orig_ndf.import_list) != _resolve_list(mod_ndf, mod_ndf.import_list):
        return True
    return _resolve_list(orig_ndf, orig_ndf.export_list) != _resolve_list(mod_ndf, mod_ndf.export_list)
    ...
    for entry_path in mod_edat.list():
        ...
        if is_ndf and orig_bytes is not None:
            ...
            try:
                ...
                if _impr_expr_differ(orig_ndf, mod_ndf):
                    log_fn(f"    IMPR/EXPR sections differ — using raw file patch")
                    ...
                    if changes:
                        ...
                        # Bytes differ but semantic diff is empty — capture raw as
                        # a safety net so nothing is silently lost.
                        warn_fn(f"  NDF diff empty but bytes differ — using raw file patch: {entry_fwd.split('/')[-1]}")
                        raw_file_patches.append({
                            "path": entry_fwd,
                            "data": base64.b64encode(mod_bytes).decode("ascii"),
                        })
...
def run_conversion(mod_folder, game_data_dir, output_rmod,
                   ...
                   log_fn, warn_fn, default_sub=None) -> bool:
