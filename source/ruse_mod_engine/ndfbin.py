    ...
    def resolve_value(self, val: NdfValue) -> Any:
        ...
        if val.type_id == T.Reference:
            ...
            if marker == TRANS_REF_MARKER:
                return f"$trans:{self.get_trans(ref)}"
...
def make_value(type_name: str, raw_value) -> NdfValue:
    ...
    # Trans reference: {"trans": N} (legacy int index) or {"trans": "name"} (portable string)
    if key == "transref":
        if not isinstance(raw_value, dict):
            raise ValueError("TransRef value must be {'trans': N} or {'trans': 'name'}")
        ref = raw_value["trans"]
        if isinstance(ref, str):
            # String form — caller must resolve to index via ensure_trans (done in _resolve_string_refs)
